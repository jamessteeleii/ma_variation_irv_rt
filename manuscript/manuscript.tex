...
\urlstyle{same}
\hypersetup{
  pdftitle={Little evidence for meaningful intervention effect heterogeneity in resistance training: Evidence from causal inference framing and meta-analytic variance comparisons},
...
in resistance training: Evidence from causal inference framing and
meta-analytic variance comparisons\thanks{Preprint, please cite as:
Steele, J. (2026). Little evidence for meaningful intervention effect
heterogeneity in resistance training: Evidence from causal inference and
meta-analytic variance comparisons. sportrxiv DOI:
...
untrained\footnote{Untrained participants given in a trained population
  ...
  interventions for at least one full year; Steele et al., 2023; Roberts
  et al., 2026) such that they are no longer experiencing meaningful
  intervention effects anyway.} participants, we can obtain some insight
...
explore both the pooled strength and pooled hypertrophy outcomes.
...
post-intervention\footnote{Note, previously in Steele et al., (2023) we
  ...
  require must be sign transformed and taken as absolute values under
  the assumption that mean-variance relationships are typically
  symmetric about zero so that absolute change scores are used. However,
  in the context of a randomised controlled trial, due to randomisation
  the post score contrast between intervention and control is an
  unbiased estimator for the average intervention effect (i.e., the
  mean) and also any variance contrast given that at post-intervention
  the variance \(\sigma\) in observed outcomes \(y_i\) is due to between
...
the population mean-variance relationship. The model was thus:
...
estimate of \(D\)\footnote{To check this assumption I compared across
  ...
  estimates (SEE PLOT).}. As such, \(\rho_{Pre:Post}\) might be
considered a reasonable estimate of \(\rho_{Int:Con}\).
...
Figure~\ref{fig-assumed-rho}, meta-analytic estimates of the difference
in \(\rho_{Pre:Post}\) between intervention and controls were \(z=\)
-0.22 {[}95\%CI: -0.73, 0.29{]} and for hypertrophy outcomes is \(z=\)
-0.38 {[}95\%CI: -0.77, 0.01{]}. For strength outcomes the estimated
\(\rho_{Pre:Post_{Int}}\) was \(r=\) 0.89 {[}95\%CI: 0.78, 0.78{]} and
\(\rho_{Pre:Post_{Con}}\) was \(r=\) 0.9 {[}95\%CI: 0.67, 0.67{]}. For
hypertrophy outcomes the estimated \(\rho_{Pre:Post_{Int}}\) was \(r=\)
0.93 {[}95\%CI: 0.87, 0.87{]} and \(\rho_{Pre:Post_{Con}}\) was \(r=\)
0.97 {[}95\%CI: 0.91, 0.91{]}.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{manuscript_files/figure-pdf/fig-assumed-rho-1.pdf}}

}

\caption{\label{fig-assumed-rho}Histogram of the assumed values of
\(\rho_{Int:Con}\) for each variance contrast in the dataset.}
...
tend towards a reduction in variance in interventions compared with
...
conclusion that there is not meaningful interindividual intervention
...
relevance as Mills et al.~(2021) have noted given that this would occur
under homogeneous treatment effects assuming that the intervention
itself changes the mean. Alternatively it may be due to ceiling effects
upon change constraining outcomes particularly given that for strength
and hypertrophy effects rapidly begin to plateau after beginning an
intervention as they are approximately logarithmic with duration of
exposure (Steele et al., 2023; Roberts et al., 2026). Or it may be that
there are indirect effects of the intervention that serve to reduce
other sources of random variation such as diet or other physical
activity during participation in a resistance training intervention
(Halliday et al., 2017).
...
inert control, assumed value of \(\rho_{Int:Con}\)) we can also make
inferences about the variance of intervention effects. A number of
methods exist to do this in the context of RCTs -- though with effective
interventions we need to account for the average intervention effects
(i.e., mean-variance relationship) -- but single trials are typically
underpowered/imprecise and thus meta-analytic methods are useful. We
can't avoid having to make some assumptions -- that's just how
scientific inference works -- but the assumptions of the variance
